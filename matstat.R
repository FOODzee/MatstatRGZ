...
# !!!
# !!! Их следует изменить так, чтобы все значения вектора npi были >= 8..10 и отличались не более чем на 0.1
# !!! и
# !!! число точек в каждой области было ~ npi.
# !!!
# min и max менять нет смысла, от них ничего не зависит.
zones <- c(min(X), 0.1173, 0.7343, 1.2657, 1.8827, max(X))
...
### Kolmogorov criteria.
diff1 <- abs(Fn(X) - F(X))      # Проверяем и справа от разрыва
diff2 <- abs(ecdf(X)(X) - F(X)) # и слева
diff <- c(diff1, diff2)
Dk <- max(diff)              # Расстояние Колмогорова.
point.idx <- which.max(diff) # Номер точки, в которой достигается максимум расстояния.

# Задаём координаты точки, в которой был достигнут максимум расстояния.
if (point.idx <= n) {
  point <- X[point.idx]
  point.val <- Fn(point)
} else {
  point <- X[point.idx - n]
  point.val <- ecdf(X)(point)
}
...
points(point, point.val, col="red")
abline(v=point, h=point.val, col="grey", lty=4)
